//Script to upload the model tum.skp from model folder
//based on modelderivative-nodejs-tutorial from Jaime Rosales D. 
//original at https://github.com/jaimerosales/modelderivative-nodejs-tutorial
...
//!!copy keys here or use environment variables!!
var CLIENT_ID = process.env.FORGE_CLIENT_ID || '<replace with your consumer key>';
var	CLIENT_SECRET = process.env.FORGE_CLIENT_SECRET || '<replace with your consumer secret>';

//simple bucket key for demo
var BUCKET_KEY = (process.env.FORGE_BUCKET_NAME || 'autodeskaec2017hackatrondata');

//simple sketchup file with tum building
var FILE_NAME = process.env.FORGE_FILE_NAME || 'tum.skp';
var FILE_PATH = process.env.FORGE_FILE_PATH || './model/tum.skp';
...
oAuth2TwoLegged.authenticate().then(function(credentials){
	...
	createBucketIfNotExist(BUCKET_KEY).then(
		...
		function(createBucketRes){
          ...
          //write key to file:
          var furnstr = "var documentId='urn:" + urnEncode + "';" 

          fs.writeFile("./www/js/fileurn.js", furnstr, function(err) {
              if(err) {
                  return logs(err);
              }
              logs("The key was saved!");
          });           
